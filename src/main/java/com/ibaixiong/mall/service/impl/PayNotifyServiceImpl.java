...
//		insert4S(orderNumber);
	...
	public void payNotifyAlipay(Map<String, String> params, PayChannel payChannel, String ip) {
		...
		System.out.println(orderNumber+","+totalFee);
		MallOrder order = checkOrderFee(orderNumber, totalFee);
		// 支付成功状态码
		orderService.updateOrderStatus(OrderStatus.PAID.getStatus(), orderNumber);
		// 修改支付变化表
...
//		insert4S(orderNumber);
	...
	@Override
	public void payNotifyBalancepay(MallOrder mallOrder,Float totalFee, PayChannel payChannel, String ip) {
		MallOrder order = checkOrderFee(mallOrder.getOrderNumber(), totalFee);
		// 修改支付变化表
		payService.notifyOrderPayAlipay(order.getOrderNumber(), null, totalFee, payChannel, null);
		// 支付成功状态码
		orderService.updateOrderStatus(OrderStatus.PAID.getStatus(), order.getOrderNumber());
		// 添加支付记录
		orderService.addOrderHistory(order.getOrderNumber(), order.getUserId(), ip, OrderStatus.PAID.getStatus(), (byte) 1);
//		insert4S(orderNumber);
	}
...
//		insert4S(orderNumber);
	...
	private MallOrder checkOrderFee(String orderNumber, float totalFee) {
		...
		System.out.println(order.getShouldPayMoney().floatValue()+","+totalFee+"-----");
		if (order.getShouldPayMoney().floatValue() != totalFee) {
			logger.info("订单金额不符:-----db:" + order.getTotalPrice() + "-----notify:" + totalFee);
