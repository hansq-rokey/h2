	$(function() {
		...
		$numTxtBox.keydown(function(e){// 绑定事件
			...
			if(!(oEvent.keyCode==46)&&!(oEvent.keyCode==8)&&!(oEvent.keyCode==9)&&!(oEvent.keyCode==37)&&!(oEvent.keyCode==39)&&!(oEvent.keyCode==110))// 键值
...
/*新面积 计算*/
    ...
    $(document).on('change', '#delim_arae,#paper_arae,#paper_h', function(event) {
        ...
        var papervalue=parseFloat($("#paper_arae").val());//发热面积
        ...
        if(hw =='' || hw == null){
        	...
        	$("#hw").css('border','1px solid #dedede');
        }
        if(isExt == 1){
	        if (delimvalue != null && papervalue != null && delimvalue!='' && papervalue!='' && hw != '' && hw !=null) {
	        	...
	        	$.ajax({
					type: "POST",
					url:"/u/product/tool/calculate.html",
					data: {formatId:formatId,groundArea:delimvalue,area:total},
					cache: false,
					async:false,
					dataType:"json",
					success: function(data) {
