    ...
    <div class="content">
        ...
        <div class="wrap">
            ...
            <div class="orderinfo-main">
                ...
                <c:if test="${bomList!=null }">
	                <div class="info info-address clearfix">
	                   <h6>物料信息</h6>
		                <div class="info-right">
		                 <div class="order-Details order-boxborder1">
							  <ul class="order-tag">
							    <li>物料名称</li>
							    <li>规格</li>
							    <li>编号</li>
							  ...
							  <c:forEach items="${bomList }" var="item">
								  <div class="order-mian">
								     ...
								     <span><fmt:formatNumber value="${item.num }" pattern="#.##"/></span>
								  </div>
							  </c:forEach>
						   </div>
					    </div>
                    ...
                    <div class="info-right">
                        <table>
                            ...
                            <tr>
                                ...
                                <td><fmt:formatNumber value="${format.price}" type="currency" maxFractionDigits="2" minFractionDigits="2" /></td>
                                <td>${num}</td>
                ...
                <div class="info-money">
                    ...
                    <p class="score"><label><input style="vertical-align: text-bottom;" id="couponmoney" type="checkbox" data-value="${couponPrice}" value="order."/>使用优惠券</label><span style="display:none">-<fmt:formatNumber value="${couponPrice}" type="currency" maxFractionDigits="2" minFractionDigits="2" /></span></p>
                    <%-- <p class="coupon1">可用优惠券：<span>-<fmt:formatNumber value="${couponPrice}" type="currency" maxFractionDigits="2" minFractionDigits="2" /></span></p> --%>
<%--                     <c:if test="${inviteCodeList!=null }">
                    ...
                    <p class="pay">应付：<span id="payprice"><fmt:formatNumber value="${totalMoney}" type="currency" maxFractionDigits="2" minFractionDigits="2" /></span></p>
                   	<input type="hidden" name="inviteCodeId" id="inviteCode" />
                    <input type="hidden" name="formatId" value="${format.id }"/>
                    <input type="hidden" name="num" value="${num }"/>
                    <input type="hidden" name="picId" value="${picId }"/>
                    <input type="hidden" name="mallCustomPicId" value="${mallCustomPicId }"/>
                ...
                <input type="hidden" id="couponMoney" name="couponMoney" value="0.00" />
                <input type="hidden" id="copeWith" name="copeWith" value="${copeWith }" />
                </form>
            </div>
        </div>
    </div>
...
function address_check(){
    ...
    $(document).ready(function() {
        ...
        $('.score label').on('click',function(){
        	...
        	var total = $("#total").val();//商品总价格
        	var copeWith = parseFloat($("#copeWith").val()).toFixed(2);//优惠后价格
        	var payprice = $("#payprice").text();//
			var couponMoney = $("#couponMoney");
        	var couponHidden = $(this).children("input").attr("data-value"); 
        	
        	if($(this).find('input').prop('checked')){
            	$("#payprice").text(copeWith);
            	couponMoney.val(couponHidden);
                $(this).siblings('span').show();
                
            }else{
            	$("#payprice").text(total);
            	couponMoney.val("0.00");
