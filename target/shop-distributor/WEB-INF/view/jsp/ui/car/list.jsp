								...
								<li class='carItemLi'>
									...
									<div class="sp-intro">
										...
										<span class="area-text">
											${item.carItemExtend.length }x${item.carItemExtend.width }x${item.carItemExtend.height }
										(m)</span>
										</c:if>
										</p>
									</div>
									...
									<div class="clearfix product-assess">
										...
										<div class="left operate-box">
											<a class="minus fl" data-id="${item.carItem.id }" data-format-id="${item.format.id }" data-ext="${item.format.isExtProperties }" data-btn="minus" href="javascript:void(0)"></a>
											<input type="text" data-id="${item.carItem.id }" name="carItemNumList" value="${item.carItem.num }" data-val="${item.carItem.num }" data-type="2" data-min="1" data-price="${item.format.productPurchasePrice }" onchange="keyup(this.value,$(this))" class="operate-text fl">
											<input type="hidden" value="${item.carItem.id }"  name="carItemIdList" />
											<span class="area-unit">${item.format.unit }</span>
											<a class="plus fl" data-id="${item.carItem.id }" data-format-id="${item.format.id }" data-ext="${item.format.isExtProperties }" data-btn="add" href="javascript:void(0)"></a>
											<!--<c:if test="${item.format.stock<item.carItem.num }">库存不足</c:if>-->
										</div>
										<c:if test="${item.format.bDisplay!=1 || item.format.stock<item.carItem.num}">
											<div class="stockbox">
												<p>库存不足</p>
				...
				<div class="no-shopping">
					<div class="no-part1 clearfix" style="margin: 120px auto; margin-left: 350px;">
						...
						<div class="fl">
							...
							<!-- <p class="clearfix"><a href="/u/product/52.html"><b>暖魔方壁挂款</b></a></p> -->
							
							<p class="clearfix"><a href="/u/product/53.html"><b>暖魔方浴室款</b></a></p>
						</div>
					</div>
					<div class="no-part2" style="height: 80px">
						<!-- <a href="/u/product/52.html"><img src="http://fe.ibaixiong.com/shop/image/noshopping2.png"></a> -->
					</div>
				</div>
				</c:if>
			</div>
			...
			settings.delAddressFn(data_id, function(data){
				if (data.success == true) {
					...
					if($('.carItemLi').length==0){
						var html='<div class="no-shopping"><div class="no-part1 clearfix"><img src="http://fe.ibaixiong.com/shop/image/noshopping1.png"><div class="fl"><p><b>购物车空空如也</b></p><p class="clearfix"><span>去了解</span><p class="clearfix"><a href="/u/product/53.html"><b>暖魔方浴室款</b></a></p></p></div></div><div class="no-part2" style="height: 80px"></div></div>'
						$('.pay-head').after(html);
						$('.new-orange-btn').hide();
					}
				}else{
