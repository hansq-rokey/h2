...
//	});
	...
	var OperationFn = function(_this, type){
		var $txtBox = _this.siblings('.operate-text'),// 输入框
			max = parseFloat($txtBox.attr('data-max')),// 允许最大值
			min = parseFloat($txtBox.attr('data-min')),// 允许最小值 默认 1
			price = $txtBox.attr('data-price');// 单价  文本框的值
		var thisExt=parseInt($(".plus").data("ext"));
		var thisStock = _this.parent().next().html();//判断库存不足是否显示
		var formatId=_this.data("format-id");
		var stock=false;
		var id=_this.attr("data-id");
		var _num = parseFloat($txtBox.val());// 计算

		if (type === 0 && thisStock == "") {//type 判断加减
			if(parseFloat($txtBox.val())>1){
				_num=_num-1;// 计算
				$.ajax({ url: "/u/car/update.html", data: "carId="+id+"&num="+_num,dataType:"json", success: function(data){
					/*console.log(data);*/
					if(!data.success){
				...
				$txtBox.val(_num.toFixed(1));// 设置数			
			}
		}else if(thisStock == ""){
				$.ajax({ url: "/u/car/update.html", data: "carId="+id+"&num="+(_num+1),dataType:"json", success: function(data){
					if(!data.success){
...
function keyup(obj,that){
	var thatvalue=that.attr("data-val");
	if(!(/^\d+\.?\d$|^\d+$/).test(obj)){
		that.val(thatvalue);
